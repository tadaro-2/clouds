...
## Аналог:
DataLens - который также ориентирован на работу с потоковыми данными и предоставляет возможности для их обработки в реальном времени.
...
## Аналог:
VDS (виртуальные выделенные серверы). С помощью этого сервиса пользователи могут арендовать виртуальные серверы с гибкой настройкой параметров, таких как объем оперативной памяти (RAM), количество виртуальных ядер процессора (CPU), объем дискового пространства и другие.
...
## Аналог:
Конкретного аналога нет, но есть сервис использующий искусственный интелект - YandexGPT 2
...
## Аналог:
Yandex SpeechKit - Этот инструмент позволяет разработчикам интегрировать функциональность обработки речи в свои приложения и сервисы (Ещё частично Yandex Dialogs - можно создавать голосовые приложения для Алисы и Умного дома Яндекса.)
...
## Аналог:
Конкретного аналога нет, но есть сервис, котрый масштабируемые вычислительные мощности для размещения - Yandex Compute Cloud
...
## Аналог:
Yandex.Cloud DNS - предоставляет средства для регистрации и управления доменами, а также для настройки DNS записей.
Он позволяет настраивать различные типы записей, такие как A, CNAME, MX, TXT и другие, для определения поведения трафика. Пользователи могут управлять записями DNS, указывая IP-адреса серверов или ресурсы, к которым должно происходить направление трафика для заданных доменов.


Заполним таблицу с исходными данными с учётом этих исследований. Результат [здесь](./aws_results.csv).

![Данные в виде таблины Excel](./Pictures/1.png)
